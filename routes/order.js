...
router.post('/order/:tableId', async (req, res) => {
  try {
    ...
    const { items = [], subtotal, barSubtotal, grandTotal, CGST, VAT, SGST, total, menuTotal, isTemporary, acPercentageAmount, isPrint, acPercentage, vatPercentage, gstPercentage } = req.body;
...
router.patch('/order/update-order-by-table/:tableId', async (req, res) => {
  try {
    const { tableId } = req.params;
    const { items = [], subtotal, CGST, SGST, total, menuTotal, isTemporary, isPrint, VAT, barSubtotal, grandTotal, acPercentageAmount, acPercentage, vatPercentage, gstPercentage, } = req.body;
...
//Update order according to orderId
router.patch('/update-order-by-id/:orderId', async (req, res) => {
  try {
    const { orderId } = req.params;
    const { items = [], subtotal, VAT, CGST, SGST, total, isTemporary, acPercentageAmount, isPrint, cashAmount, onlinePaymentAmount, dueAmount, complimentaryAmount, discount, barSubtotal, menuTotal, grandTotal, acPercentage, vatPercentage, gstPercentage, } = req.body;
...
router.get('/latest-orders', async (req, res) => {
  try {
    ...
    const latestOrders = await Order.find({
      ...
      // .limit(all); // Limit the number of results to 10
